...
const StudentLogin = () => {
  ...
  const StudentLogin = async () => {
    try {
      ...
      const response = await fetch(`${Links.Domain}/api/User/Student_login`, {
        method: 'POST',
        headers: {
      ...
      if (responseJson[0].id === 1) {
        Alert.alert("Your welcome dear student.");
        await AsyncStorage.setItem("Type", "Student");
        await AsyncStorage.setItem("Login", "1");
        await AsyncStorage.setItem("Email", email);
        await AsyncStorage.setItem("Password", password);
        setLoading(false);
        navigation.navigate("StudydoorMain", { Email: `${email}`, Password: `${password}` });
      } else {
        setLoading(false);
            ...
            <View style={[styles.logoContainer, { overflow: 'visible' }]}>
              ...
              <View style={{ position: 'absolute', bottom: -140, alignSelf: 'center', justifyContent: "center" }}>
                <View style={{ backgroundColor: Color.Color.bottomtabBackground, width: 80, height: 80, borderRadius: 100, elevation: 20, alignSelf: 'center', justifyContent: "center" }}>
                  <Image source={require('../../assets/Student.png')} style={{ width: 50, height: 50, alignSelf: 'center' }} />
                </View>
                <Text style={{ fontSize: 22, color: Color.Color.bottomtabBackground, fontWeight: "700" }}>Student</Text>
              </View>
            </View>
          </View>
          {loading ? (
            ...
            <View style={[styles.mainContainer, { marginTop: 60 }]}>
              <View style={styles.inputContainer}>
                ...
                <TouchableOpacity style={styles.loginButton} onPress={() => { StudentLogin(); }}
                  disabled={email === '' || password === '' || emailErrorText === 'red'}
                >
              ...
              <View style={{ alignSelf: 'center', marginTop: 20 }}>
                <Text style={{ color: "#FFA836" }} onPress={() => { navigation.navigate("InstituteLogin") }} > Are you an Institute? </Text>
              </View>
            </View>
          )}
        </View>
