...
const FacultyLogin = () => {
  ...
  const FacultyLoginHandler = async () => {
    try {
      ...
      const response = await fetch(`${Links.Domain}/api/User/Faculty_login`, {
        method: 'POST',
        headers: {
      ...
      if (responseJson[0].id === 1) {
        Alert.alert("We welcome you Faculty.");
        await AsyncStorage.setItem("Type", "Faculty");
      ...
      <ScrollView>
            ...
            <View style={[styles.logoContainer, { overflow: 'visible' }]}>
              ...
              <View style={{ position: 'absolute', bottom: -140, alignSelf: 'center', justifyContent: "center" }}>
                <View style={{ backgroundColor: Color.Color.bottomtabBackground, width: 80, height: 80, borderRadius: 100, elevation: 20, alignSelf: 'center', justifyContent: "center" }}>
                  <Image source={require('../assets/Faculty.png')} style={{ width: 50, height: 50, alignSelf: 'center' }} />
                </View>
                <Text style={{ fontSize: 22, color: Color.Color.bottomtabBackground, fontWeight: "700" }}>Faculty</Text>
              </View>
            </View>
          </View>
          {loading ? (
            ...
            <View style={[styles.mainContainer, { marginTop: 60 }]}>
              <View style={styles.inputContainer}>
                ...
                <Input placeholder="Password"
                  ...
                  style={styles.inputBox}
                />
                <TouchableOpacity style={styles.loginButton} onPress={() => { FacultyLoginHandler() }} disabled={email === '' || password === '' || emailErrorText === 'red'} >
        ...
        <Text style={{ color: "#0247fe" }} onPress={() => {navigation.navigate("ChoiseForCreateAccount")}} > Create account</Text>
              ...
              <View style={{ alignSelf: 'center', marginTop: 20,flexDirection:"row" }}>
                <Text style={{ color: "#FFA836" }} onPress={() => { navigation.navigate("StudentLogin") }} > Are you a student? </Text>
                
          <Text style={{ color: "blue" }} onPress={() => {navigation.navigate("InstituteLogin")}} >or institute? </Text>
              </View>
            </View>
          )}
        </View>
